...
def zeep_pythonvalue(self, xmlvalue):      DELETE
    return xmlvalue
...
#Создание гистограммы камера 42
#42
...
for i,col in enumerate(color): #each value corresponds to number of pixels in that imaging with its corresponding pixel value RGB
    histogr = cv2.calchistog([frame_42],[i],None,[256],[0,256])
    plt.plot(histogr,color = col)
    plt.xlim([0,256])
    
#Создаём график для 43 камеры
ret,frame_43=stream_43.read()
hsv = cv2.cvtColor(frame_43,cv2.COLOR_BGR2HSV)
histog = cv2.calchistog([hsv], [0,1], None, [180, 256], [0, 180, 0, 256]) #оттенок 180 насыщенность 256
fig.add_subplot(4, 2, 3)
plt.imshow(histog,interpolation = 'nearest')
    
fig.add_subplot(4, 2, 4)
for i,col in enumerate(color): #each value corresponds to number of pixels in that imaging with its corresponding pixel value RGB
    histogr = cv2.calchistog([frame_43],[i],None,[256],[0,256])
    plt.plot(histogr,color = col)
    plt.xlim([0,256])    
...
#Создание гистограммы для 42 камеры
#42
ret,frame_42=stream_42.read()
hsv = cv2.cvtColor(frame_42,cv2.COLOR_BGR2HSV)
histog = cv2.calchistog([hsv], [0,1], None, [180, 256], [0, 180, 0, 256]) #hue 180 saturation 256
...
#Создание гистограммы для 43 камеры
#43
ret,frame_43=stream_43.read()
hsv = cv2.cvtColor(frame_43,cv2.COLOR_BGR2HSV)
histog = cv2.calchistog([hsv], [0,1], None, [180, 256], [0, 180, 0, 256]) #hue 180 saturation 256
